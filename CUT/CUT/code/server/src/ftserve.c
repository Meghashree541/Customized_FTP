/*
*FTP_SERVER
* Server side connection is established.
* User authentication and commands like ls, pwd, get, put and bye defined.
*/

#include "../include/ftserve.h"
int flag=0;
...
void ftserve_retr(int sock_control, int sock_data, char* filename)
{
	...
	if (!fd) {	
		...
		if(flag==1) send_response(sock_control, 5);
		else send_response(sock_control, 226);
		fclose(fd);
	}
//   else{
// 	  send_response(sock_control, 5);
//   }
}

/**
 * Stores file specified in filename over data connection, sending
 * control message over control connection
 * Handles case of null or invalid filename
 */

int ftserve_put( int sock_control,int sock_data, char* filename)
{
	char data[MAXSIZE];
    	int size;
	if(flag==1)
	{
	  	if(chdir("/home/cg84-user12/CUT/CUT/code/server/src/var/ftp/upload")<0)
...
int ftserve_pwd(int sock_data, int sock_control)
{
	...
	send_response(sock_control, 257);	// send 257
	...
	while ((num_read = getline(&line, &len, fd)) != -1) {
		...
		if (pch != NULL) {
		  ...
		  if(strcmp(pass,"")==0){
                        ...
                        if(chdir("../src/var/ftp/pub")==0)
                        {
                               flag=1;
			       auth=1;
                               break;
                        }
			perror("chdir() cannot be created\n");
                  }
		...
		trimstr(password, (int)strlen(password));
		  ...
		  {
				 char path[MAXSIZE]="../src/";
                                 if(chdir(strcat(path,user))==0)
                                 {
                                         auth=1;
					 break;
				 }
...
{	
	...
	if (strcmp(cmd, "QUIT")==0) {
		rc = 221;
	} else if((strcmp(cmd, "USER")==0) ||(strcmp(cmd, "PWD")==0) || (strcmp(cmd, "PASS")==0) ||(strcmp(cmd, "LIST")==0) || (strcmp(cmd, "RETR")==0) || (strcmp(cmd,"STOR")==0)) {
		rc = 200;
	}
